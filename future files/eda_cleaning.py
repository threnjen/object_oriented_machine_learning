...
class EDACleaning():
    ...
    def drop_dupes(self, df:pd.DataFrame, subset:list=None):
        if subset:
    ...
    def remove_outliers(self, df, fields:list, method:str, range:float):
        if method=='iqr':
            for field in fields:
                lower_range, upper_range = self._calculate_iqr(df[field], range)
                df.drop(df[ (df[field] > upper_range) | (df[field] < lower_range) ].index , inplace=True)
        return df
    ...
    def print_sorted(self, df, field=None, asc:bool = False, groupby = None):
        if groupby:
    ...
    def reset_df_index(self, df) -> pd.DataFrame:
    ...
    def show_correlated_pairs(self, df):
    ...
    def show_target_correlation(self, df):
        ...
        df_correlations = df.corr().abs().stack().reset_index().sort_values(0, ascending=False)
        df_correlations.loc[df_correlations['level_0'] == self.target].sort_values(0, ascending=False)
