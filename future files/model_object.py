...
class BaseModel(ABC):
    ...
    def __init__(self, filename:str, target:str, seed:Optional[int]=None):
    ...
    def remove_outliers(self, fields:list = [], method:str='iqr', range:float=1.5):
        self.df = self.cleaner.remove_outliers(self.df, fields, method, range)

    def _calculate_iqr(self, column, range):
        self.cleaner(column, range)
